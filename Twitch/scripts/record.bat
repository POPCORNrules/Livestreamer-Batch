...
:Current
cls
IF NOT EXIST "%TP%\Twitch\%channel%" (
mkdir "%TP%\Twitch\%channel%"
set "save=%TP%\Twitch\%channel%"
) else (
set "save=%TP%\Twitch\%channel%"
)
goto video
...
:end
cls
start cmd /c %TP%\Twitch\scripts\play.bat
if not x%channel:http=%==x%channel% (
  echo This is a URL, trying to open %channel%
  livestreamer %channel% %quality% -o "%save%\%channel%_%mydate%_%mytime%.mp4"
)else (
livestreamer.exe twitch.tv/%channel% %quality% -l none -o "%save%\%channel%_%mydate%_%mytime%.mp4"
)
cls
set "svdir=%save%\%channel%_%mydate%_%mytime%.mp4"
